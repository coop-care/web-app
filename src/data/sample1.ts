import * as Store from "../store/index";

export default [
  {
    id: "4s62btq9",
    name: "Emma Becker",
    problems: [
      {
        assessment: [] as Store.Note[],
        problem: {
          id: "12",
          scope: 0,
          severity: 2,
          signsAndSymptoms: [
            {
              id: "03"
            },
            {
              id: "13"
            },
            {
              id: "14"
            },
            {
              id: "15"
            },
            {
              id: "09"
            },
            {
              id: "17"
            },
            {
              id: "18"
            },
            {
              id: "22"
            }
          ],
          otherSignsAndSymptoms: "",
          details: "",
          isHighPriority: true
        },
        interventions: [
          {
            category: {
              id: "01"
            },
            target: {
              id: "28"
            },
            details: [
              {
                text: "angefangen eine therapeutische Beziehung aufzubauen",
                createdAt: new Date("2019-06-11T00:00:00.000Z")
              }
            ],
            startedAt: new Date("2019-06-11T00:00:00.000Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "01"
            },
            target: {
              id: "49"
            },
            details: [
              {
                text: "Symptome überprüft",
                createdAt: new Date("2019-06-11T00:00:00.000Z")
              }
            ],
            startedAt: new Date("2019-06-11T00:00:00.000Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "03"
            },
            target: {
              id: "31"
            },
            details: [
              {
                text: "Termin zur Einschätzung geplant",
                createdAt: new Date("2019-06-11T00:00:00.000Z")
              }
            ],
            startedAt: new Date("2019-06-11T00:00:00.000Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "04"
            },
            target: {
              id: "49"
            },
            details: [
              {
                text: "Aktivität, Stimmungsschwankungen, Halluzinationen und Pflegeberichte",
                createdAt: new Date("2019-06-11T00:00:00.000Z")
              }
            ],
            startedAt: new Date("2019-06-11T00:00:00.000Z"),
            endedAt: undefined
          }
        ],
        outcomes: [
          {
            knowledge: {
              observation: 2,
              expectation: 4,
              comment: "anhaltende Symptome störten Realitätswahrnehmung"
            },
            behaviour: {
              observation: 2,
              expectation: 4,
              comment: "hörte selten auf zu suchen und zu hetzen, aber aß und schlief"
            },
            status: {
              observation: 2,
              expectation: 4,
              comment: "Unruhe, beschrieb Wahnvorstellungen und Halluzinationen"
            },
            personRatedInPlaceOfOwner: "",
            createdAt: new Date("2019-06-11T00:00:00.000Z")
          },
          {
            status: {
              observation: 2,
              expectation: 4,
              comment: ""
            },
            knowledge: {
              observation: 2,
              expectation: 4,
              comment: ""
            },
            behaviour: {
              observation: 2,
              expectation: 4,
              comment: ""
            },
            personRatedInPlaceOfOwner: "",
            createdAt: new Date("2019-07-09T00:00:00.000Z")
          },
          {
            status: {
              observation: 2,
              expectation: 4,
              comment: ""
            },
            knowledge: {
              observation: 3,
              expectation: 4,
              comment: ""
            },
            behaviour: {
              observation: 2,
              expectation: 4,
              comment: ""
            },
            personRatedInPlaceOfOwner: "",
            createdAt: new Date("2019-08-06T00:00:00.000Z")
          },
          {
            status: {
              observation: 2,
              expectation: 4,
              comment: ""
            },
            knowledge: {
              observation: 3,
              expectation: 4,
              comment: ""
            },
            behaviour: {
              observation: 3,
              expectation: 4,
              comment: ""
            },
            personRatedInPlaceOfOwner: "",
            createdAt: new Date("2019-09-03T00:00:00.000Z")
          },
          {
            status: {
              observation: 3,
              expectation: 4,
              comment: ""
            },
            knowledge: {
              observation: 4,
              expectation: 4,
              comment: ""
            },
            behaviour: {
              observation: 3,
              expectation: 4,
              comment: ""
            },
            personRatedInPlaceOfOwner: "",
            createdAt: new Date("2019-10-01T00:00:00.000Z")
          },
          {
            status: {
              observation: 3,
              expectation: 4,
              comment: ""
            },
            knowledge: {
              observation: 4,
              expectation: 4,
              comment: ""
            },
            behaviour: {
              observation: 4,
              expectation: 4,
              comment: ""
            },
            personRatedInPlaceOfOwner: "",
            createdAt: new Date("2019-10-29T00:00:00.000Z")
          }
        ],
        createdAt: new Date("2019-06-11T00:00:00.000Z"),
        resolvedAt: undefined,
        ratingIntervalInDays: 28
      },
      {
        assessment: [] as Store.Note[],
        problem: {
          id: "38",
          scope: 0,
          severity: 2,
          signsAndSymptoms: [
            {
              id: "01"
            },
            {
              id: "02"
            },
            {
              id: "03"
            },
            {
              id: "04"
            },
            {
              id: "05"
            },
            {
              id: "10"
            }
          ],
          otherSignsAndSymptoms: "",
          details: "",
          isHighPriority: true
        },
        interventions: [
          {
            category: {
              id: "01"
            },
            target: {
              id: "41"
            },
            details: [
              {
                text: "was sie tun kann und was nicht",
                createdAt: new Date("2019-11-21T21:40:18.274Z")
              }
            ],
            startedAt: new Date("2019-11-21T21:41:51.957Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "03"
            },
            target: {
              id: "77"
            },
            details: [
              {
                text: "Vermittlung von Hilfe bei Dusche, Umziehen und Wäsche",
                createdAt: new Date("2019-11-21T21:41:14.572Z")
              }
            ],
            startedAt: new Date("2019-11-21T21:41:51.957Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "04"
            },
            target: {
              id: "41"
            },
            details: [
              {
                text: "Sauberkeit, Angemessenheit der Hilfeleistung",
                createdAt: new Date("2019-11-21T21:41:42.763Z")
              }
            ],
            startedAt: new Date("2019-11-21T21:41:51.957Z"),
            endedAt: undefined
          }
        ],
        outcomes: [
          {
            createdAt: new Date("2019-11-21T21:41:51.957Z"),
            knowledge: {
              observation: 2,
              expectation: 3,
              comment: "erkannte Bedarf an Körperhygiene"
            },
            behaviour: {
              observation: 1,
              expectation: 3,
              comment: "kann die Körperpflege nicht starten oder beenden"
            },
            status: {
              observation: 1,
              expectation: 4,
              comment: "Körper und Kleidung unsauber"
            },
            personRatedInPlaceOfOwner: ""
          }
        ],
        createdAt: new Date("2019-11-21T21:41:51.957Z"),
        resolvedAt: undefined,
        ratingIntervalInDays: 28
      },
      {
        assessment: [] as Store.Note[],
        problem: {
          id: "42",
          scope: 0,
          severity: 2,
          signsAndSymptoms: [
            {
              id: "03"
            },
            {
              id: "08"
            }
          ],
          otherSignsAndSymptoms: "",
          details: "",
          isHighPriority: true
        },
        interventions: [
          {
            category: {
              id: "01"
            },
            target: {
              id: "32"
            },
            details: [
              {
                text: "weniger Koffein trinken, weil es die Wirksamkeit psychotroper Medikamente verringert",
                createdAt: new Date("2019-11-21T21:46:28.059Z")
              }
            ],
            startedAt: new Date("2019-11-21T21:53:44.870Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "01"
            },
            target: {
              id: "33"
            },
            details: [
              {
                text: "muss alle Dosen wie geplant einnehmen",
                createdAt: new Date("2019-11-21T21:47:35.850Z")
              }
            ],
            startedAt: new Date("2019-11-21T21:53:44.870Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "03"
            },
            target: {
              id: "31"
            },
            details: [
              {
                text: "Terminplanung zur Überprüfung von Medikamenten",
                createdAt: new Date("2019-11-21T21:48:38.586Z")
              }
            ],
            startedAt: new Date("2019-11-21T21:53:44.870Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "03"
            },
            target: {
              id: "77"
            },
            details: [
              {
                text: "Psychiatriefachkraft wird Emma helfen",
                createdAt: new Date("2019-11-21T21:49:35.282Z")
              }
            ],
            startedAt: new Date("2019-11-21T21:53:44.870Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "04"
            },
            target: {
              id: "33"
            },
            details: [
              {
                text: "Berichte von Kundin, Psychiater und Verwaltung",
                createdAt: new Date("2019-11-21T21:53:14.345Z")
              }
            ],
            startedAt: new Date("2019-11-21T21:53:44.870Z"),
            endedAt: undefined
          }
        ],
        outcomes: [
          {
            createdAt: new Date("2019-11-21T21:53:44.870Z"),
            knowledge: {
              observation: 3,
              expectation: 4,
              comment: "kannte Namen von Medikamenten und dass sie genommen werden müssen"
            },
            behaviour: {
              observation: 2,
              expectation: 4,
              comment: "hat einige Medikamente ausgelassen; benötigt eine konsistente Verabreichung"
            },
            status: {
              observation: 2,
              expectation: 3,
              comment: "Symptome einer psychischen Erkrankung offensichtlich"
            },
            personRatedInPlaceOfOwner: ""
          }
        ],
        createdAt: new Date("2019-11-21T21:53:44.870Z"),
        resolvedAt: undefined,
        ratingIntervalInDays: 28
      }
    ],
    createdAt: new Date("2019-06-11T00:00:00.000Z")
  },
  {
    id: "s4fvng9p",
    name: "Francis Richter",
    problems: [
      {
        assessment: [] as Store.Note[],
        problem: {
          id: "07",
          scope: 0,
          severity: 1,
          signsAndSymptoms: [],
          otherSignsAndSymptoms: "",
          details: "Francis lebt allein und ist im Seniorenalter, gab aber an, dass ihr Unterstützungssystem angemessen ist.",
          isHighPriority: false
        },
        interventions: [],
        outcomes: [],
        createdAt: new Date("2019-11-25T18:33:51.943Z"),
        resolvedAt: undefined,
        ratingIntervalInDays: 28
      },
      {
        assessment: [] as Store.Note[],
        problem: {
          id: "23",
          scope: 0,
          severity: 1,
          signsAndSymptoms: [],
          otherSignsAndSymptoms: "",
          details: "Francis Urteilsvermögen und Gedächtnis schienen ausreichend, abgesehen von einigen Problemen bei der planmäßigen Einnahme von Medikamenten. \nWerde bei Bedarf mit den Töchtern Kontakt aufnehmen.",
          isHighPriority: false
        },
        interventions: [],
        outcomes: [],
        createdAt: new Date("2019-11-25T18:34:55.223Z"),
        resolvedAt: undefined,
        ratingIntervalInDays: 28
      },
      {
        assessment: [] as Store.Note[],
        problem: {
          id: "29",
          scope: 0,
          severity: 2,
          signsAndSymptoms: [
            {
              id: "01"
            },
            {
              id: "08"
            },
            {
              id: "10"
            },
            {
              id: "11"
            },
            {
              id: "17"
            }
          ],
          otherSignsAndSymptoms: "",
          details: "",
          isHighPriority: true
        },
        interventions: [
          {
            category: {
              id: "01"
            },
            target: {
              id: "07"
            },
            details: [
              {
                text: "Linderung von Ödemen, Erhöhung der Beine, Anstrengung reduzieren, Verbesserung des Schlafes, Planung der Flüssigkeitsaufnahme",
                createdAt: new Date("2019-11-25T18:38:53.610Z")
              }
            ],
            startedAt: new Date("2019-11-25T18:41:07.807Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "01"
            },
            target: {
              id: "67"
            },
            details: [
              {
                text: "einige eisenreiche Lebensmittel essen",
                createdAt: new Date("2019-11-25T18:39:19.966Z")
              }
            ],
            startedAt: new Date("2019-11-25T18:41:07.807Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "01"
            },
            target: {
              id: "50"
            },
            details: [
              {
                text: "täglich Körpergewicht messen, Symptomprotokoll, wann sind die Dienstleister zu benachrichtigen",
                createdAt: new Date("2019-11-25T18:39:53.047Z")
              }
            ],
            startedAt: new Date("2019-11-25T18:41:07.807Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "03"
            },
            target: {
              id: "67"
            },
            details: [
              {
                text: "arrangierte Essen auf Rädern ab dem nächsten Tag",
                createdAt: new Date("2019-11-25T18:40:18.774Z")
              }
            ],
            startedAt: new Date("2019-11-25T18:41:07.807Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "04"
            },
            target: {
              id: "50"
            },
            details: [
              {
                text: "Vitalwerte, Veränderungen in Schweregrad und Status",
                createdAt: new Date("2019-11-25T18:40:38.599Z")
              }
            ],
            startedAt: new Date("2019-11-25T18:41:07.807Z"),
            endedAt: undefined
          }
        ],
        outcomes: [
          {
            createdAt: new Date("2019-11-25T18:41:07.807Z"),
            knowledge: {
              observation: 2,
              expectation: 4,
              comment: "hat eine Verschlechterung des Zustands nicht erkannt"
            },
            behaviour: {
              observation: 2,
              expectation: 4,
              comment: "misst weder ihr Körpergewicht, noch legt sie die Beine hoch oder meldet Anzeichen oder Symptome"
            },
            status: {
              observation: 2,
              expectation: 3,
              comment: "signifikante Müdigkeit, Ödeme in den unteren Extremitäten, erhöhte Herzfrequenz, Dyspnoe"
            },
            personRatedInPlaceOfOwner: ""
          }
        ],
        createdAt: new Date("2019-11-25T18:41:07.807Z"),
        resolvedAt: undefined,
        ratingIntervalInDays: 28
      },
      {
        assessment: [] as Store.Note[],
        problem: {
          id: "42",
          scope: 0,
          severity: 2,
          signsAndSymptoms: [
            {
              id: "01"
            },
            {
              id: "03"
            }
          ],
          otherSignsAndSymptoms: "",
          details: "",
          isHighPriority: true
        },
        interventions: [
          {
            category: {
              id: "01"
            },
            target: {
              id: "32"
            },
            details: [
              {
                text: "Zweck, was ist zu erwarten oder zu berichten",
                createdAt: new Date("2019-11-25T18:44:14.309Z")
              }
            ],
            startedAt: new Date("2019-11-25T18:45:19.054Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "01"
            },
            target: {
              id: "34"
            },
            details: [
              {
                text: "Benutzung, Vorteile und Möglichkeiten eines Tablettenorganisators",
                createdAt: new Date("2019-11-25T18:44:24.773Z")
              }
            ],
            startedAt: new Date("2019-11-25T18:45:19.054Z"),
            endedAt: undefined
          },
          {
            category: {
              id: "04"
            },
            target: {
              id: "32"
            },
            details: [
              {
                text: "wie vorgeschrieben einnehmen, Notizen machen",
                createdAt: new Date("2019-11-25T18:44:55.134Z")
              }
            ],
            startedAt: new Date("2019-11-25T18:45:19.054Z"),
            endedAt: undefined
          }
        ],
        outcomes: [
          {
            createdAt: new Date("2019-11-25T18:45:19.054Z"),
            knowledge: {
              observation: 2,
              expectation: 4,
              comment: "wusste nicht, welchen Zweck oder welche Nebenwirkungen die Medikamente haben"
            },
            behaviour: {
              observation: 3,
              expectation: 5,
              comment: "stimmte zu, dass sie ein Erinnerungssystem braucht"
            },
            status: {
              observation: 2,
              expectation: 5,
              comment: "zwei kürzliche Krankenhausaufenthalte"
            },
            personRatedInPlaceOfOwner: ""
          }
        ],
        createdAt: new Date("2019-11-25T18:45:19.054Z"),
        resolvedAt: undefined,
        ratingIntervalInDays: 28
      }
    ],
    createdAt: new Date("2019-11-25T18:12:32.060Z")
  }
] as Store.Customer[];
